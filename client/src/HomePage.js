...
class HomePage extends Component {
	...
	componentWillMount(){
		...
		.then(res => {
			...
			this.setState({statChartData: statChartData, statMax: statChartData.impressionsMax, statAvg: Math.ceil(statChartData.impressionsTotal/res.length), statChartLoaded: true})
		})
		.catch(error => {
		...
		.then(res => {
			...
			let eventChartData = res.map(event => {
				...
				event.events = parseInt(event.events)
				max = Math.max(event.events, max)
				total += event.events
				return event
			})
			this.setState({eventChartData: eventChartData, eventMax: max, eventAvg: Math.ceil(total/res.length), eventChartLoaded: true})
		})
		.catch(error => {
	...
	handleSort = clickedColumn => () => {
	  //sort the entire data set, reset page
	  ...
	  //mew column is ascending ==> same column, reverse it
		...
		return (
		  <div>
		      ...
		      <Grid container stackable>
		        <Grid.Row>
		          <Grid.Column width={8}>
		            ...
		            <p>
		              Here's example of event data for a particular week.
		            </p>
		            {eventDailyError &&  <Message negative>
		                <Message.Header>Sorry!</Message.Header>
		                <p>{eventDailyErrorMsg}</p>
		              </Message>}
		            {!eventChartLoaded && !eventDailyError && <Header textAlign="center" as='h1'><Icon loading name='spinner' /></Header>}
		            {eventChartLoaded && <ResponsiveContainer width='100%' aspect={4.0/3.0}><BarChart data={eventChartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
		          ...
		          <Grid.Column width={8}>
			          ...
			          <p>
			            And here is some more detailed data from that same week.
			          </p>
			            <Button.Group fluid>
			            ...
			            {statDailyError &&  <Message negative>
			                <Message.Header>Sorry!</Message.Header>
			                <p>{statDailyErrorMsg}</p>
			              </Message>}
		          	{!statChartLoaded && !statDailyError && <Header as='h1' textAlign="center"><Icon loading name='spinner' /></Header>}
		          	{statChartLoaded && <ResponsiveContainer width='100%' aspect={4.0/3.0}><LineChart data={statChartData} margin={{ top: 5, right: 20, bottom: 5, left: 20 }}>
		    ...
		    <Segment style={{ padding: '5em 0em' }} vertical>
		      <Container text>
		      {statHourlyError &&  <Message negative>
		          <Message.Header>Sorry!</Message.Header>
